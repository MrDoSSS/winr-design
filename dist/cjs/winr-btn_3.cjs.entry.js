...
const WinrBtn = class {
  ...
  render() {
    return (index.h("button", { class: { [this.kind]: true, loading: this.loading }, type: this.type, disabled: this.disabled }, index.h("span", null, index.h("slot", null))));
  }
};
...
const WinrInput = class {
  ...
  validate(value) {
    ...
    this.validateTimeout = window.setTimeout(() => {
      this.innerValidators.forEach(validator$1 => {
        validator.validators[validator$1.name].validate(value, this.el.id, validator$1);
      });
    }, 300);
  }
  validateResult(e) {
